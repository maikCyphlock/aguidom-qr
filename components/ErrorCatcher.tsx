...
export function ErrorCatcher() {
  ...
  const showFeedbackToast = (friendlyMessage: string, payloadBase: Omit<FeedbackData, "timestamp">) => {
      ...
      (t) => (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
          ...
          <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">
            {friendlyMessage}
            {payloadBase.issueType === "error" && errorBufferRef.current.length > 1 ? (
              <span> Hemos registrado {errorBufferRef.current.length} errores.</span>
            ) : null}
          </p>
            ...
            <Button
              ...
              onClick={() => {
                const finalPayload: FeedbackData = {
                  ...
                  userNote,
                  errors: payloadBase.issueType === "error" ? [...errorBufferRef.current] : undefined,
                };
