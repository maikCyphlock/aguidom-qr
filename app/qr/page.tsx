...
export default async function Page() {
	...
	const mainProgram = Effect.gen(function* () {
		...
		const user = yield* Effect.tryPromise({
			try: () => db.select().from(users).where(eq(users.email, email)).get(),
				...
				new Error(
					`Error al buscar usuario: ${error instanceof Error ? error.message : String(error)}`,
				),
		});
		...
		const club = yield* Effect.tryPromise({
			try: () => {
				...
				return db.select().from(clubs).where(eq(clubs.id, user.clubId)).get();
			},
			catch: (error) =>
				new Error(
					`Error al buscar el club: ${error instanceof Error ? error.message : "Error desconocido"}`,
				),
		});
	...
	const getFriendlyErrorMessage = (error: unknown): string => {
		...
		if (
			error instanceof Error &&
			error.message.includes("Error al buscar usuario")
		) {
		...
		if (
			error instanceof Error &&
			error.message.includes("Tu cuenta no está asociada a ningún club")
		) {
		...
		if (
			error instanceof Error &&
			error.message.includes("Error al buscar el club")
		) {
