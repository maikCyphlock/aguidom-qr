...
const attendancePageLogic = Effect.gen(function* () {
  ...
  const asistencias = yield* Effect.promise(() =>
    db
      .select()
      .from(qrTokens)
      .where(eq(qrTokens.userId, data.user?.id))
  );
  ...
  return (
    <DashboardSidebar userFromDb={userFromDb} claims={claims}>
      <div className="space-y-6">
        {/* Encabezado */}
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Mis Registros de Asistencia</h1>
              ...
              >
                ...
                <CardContent className="flex flex-col gap-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-muted-foreground">
                      {asistencia.scannedAt
                        ? new Date(asistencia.scannedAt).toLocaleString("es-ES", {
                            day: "2-digit",
                            month: "short",
                            year: "numeric",
                            hour: "2-digit",
