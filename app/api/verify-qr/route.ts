...
const handleQrVerification = (request: NextRequest) =>
  Effect.gen(function* ($) {
    ...
    if (!SECRET_KEY || SECRET_KEY.length < 32) {
      ...
      return NextResponse.json({ error: "Configuración inválida: JWT_SECRET_KEY no está configurada correctamente" }, { status: 500 });
    }
    ...
    const { payload } = yield* $(Effect.tryPromise({
      try: () => jwtVerify(token, secret, { algorithms: ["HS256"], clockTolerance: 15 }),
      catch: (err) => {
    ...
    const usersResult = yield* $(Effect.tryPromise({
    ...
    const existing = yield* $(Effect.tryPromise({
      try: () => db.select().from(qrTokens).where(
        and(
          eq(qrTokens.token, token),
          isNull(qrTokens.scannedAt)
        )
      ),
      catch: () => new Error("Error al buscar en la base de datos")
    }));
    ...
    console.log("[QR VERIFY] 🟢 Token válido. Procediendo a marcar como escaneado...");
    ...
    Effect.catchTag("Error", (e) => {
      ...
      if (e.message.includes("Token inválido")) {
        status = 400;
        errorMessage = "El token no es un JWT válido o tiene un formato incorrecto.";
      ...
      return Effect.succeed(NextResponse.json({ saved: false, error: errorMessage }, { status }));
    }),
    Effect.catchAll(() =>
      Effect.succeed(NextResponse.json({ saved: false, error: "Error interno del servidor" }, { status: 500 }))
    ),
