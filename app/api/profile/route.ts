...
function getRootErrorMessage(err: unknown): string | undefined {
  try {
    ...
    const walk = (x: any): string | undefined => {
      if (!x || typeof x !== "object") return typeof x === "string" ? x : undefined;
      ...
      for (const k of Object.getOwnPropertyNames(x)) {
        if (k.toLowerCase().includes("cause") || k.toLowerCase().includes("error")) {
          const m = walk(x[k]);
          if (m) return m;
        }
      }
...
/** Effects */
const updateProfileEffect = (userId: string, name: string, idNumber: string, phone?: string) =>
  Effect.tryPromise({
    try: () =>
      db
        .update(users)
      ...
      if (root.includes("UNIQUE constraint failed") && root.includes("users.id_number")) {
  ...
  const handled = Effect.matchEffect(program, {
    ...
    onFailure: (cause) => {
      ...
      if (root.includes("USER_NOT_LOGGED_IN")) {
        return Effect.succeed(NextResponse.json({ error: "User not logged in" }, { status: 401 }));
      }
      if (root.includes("ID_NUMBER_CONFLICT")) {
        return Effect.succeed(NextResponse.json({ error: "El número de identificación ya está en uso." }, { status: 409 }));
      }
      if (root.includes("CREATE_CLIENT_ERROR") || root.includes("GET_USER_ERROR") || root.includes("DB_ERROR")) {
        return Effect.succeed(
          NextResponse.json({ error: "Ha ocurrido un error interno. Intenta nuevamente más tarde." }, { status: 500 })
        );
      }
      ...
      try {
        if (Cause.isDieType(cause)) {
          ...
          return Effect.succeed(
            NextResponse.json({ error: "Error crítico en el servidor. El equipo fue notificado." }, { status: 500 })
          );
        }
      } catch {
        // ignore
