...
import { Users, QrCode, Calendar, CheckCircle, UserCheck, Search } from "lucide-react";
...
export default function ClubClient({ club, attendances, members }: { club: Club; attendances: AttendanceItem[]; members: Member[] }) {
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-red-600">{Math.max(totalMembers - totalPresent, 0)}</div>
                </CardContent>
              </Card>
              <Card>
                <CardHeader className="pb-3">
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-black">
                    {totalMembers > 0 ? Math.round((totalPresent / totalMembers) * 100) : 0}%
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          <div className="space-y-6">
            <Card>
              ...
              <CardContent className="text-center py-12">
                ...
                <p className="text-sm text-gray-500 mt-4">Posiciona el código QR frente a la cámara</p>
              </CardContent>
            </Card>
          </div>
        )}
                  ...
                  {attendances.map((record) => (
                    <div key={record.attendanceId} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-900">{record.userName ?? "Desconocido"}</p>
                          <p className="text-sm text-gray-500">{club.name}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-medium text-gray-900">{formatTime(record.scanTime)}</p>
                        <p className="text-sm text-green-600">Acceso autorizado</p>
                      </div>
                    </div>
                  ))}
                  {attendances.length === 0 && (
                    <p className="text-sm text-gray-500">No hay asistencias registradas aún.</p>
                  )}
                </div>
              </CardContent>
            </Card>
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {members.filter((m) => (m.name ?? "").toLowerCase().includes(searchTerm.toLowerCase())).length} de {members.length} miembros
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  {members
                    ...
                    <div key={member.userId} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-900">{member.name ?? "Sin nombre"}</p>
                          <p className="text-sm text-gray-500">{club.name}</p>
                        </div>
                      </div>
